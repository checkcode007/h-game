 ...
 */
@Component
public class CodeSendList implements IHandler<User.C_10407> {
    ...
    public AbstractMessageLite handle(ChannelHandlerContext ctx, MyMessage.MyMsgReq msgReq) throws Exception {
        ...
        User.C_10407 req = User.C_10407.parseFrom(ByteString.copyFrom(list).toByteArray());
        return handleDo(ctx, req);
    }

    @Override
    public AbstractMessageLite handleDo(ChannelHandlerContext ctx, User.C_10407 req) {
        long uid = ctx.channel().attr(ChannelAttributes.USER_ID).get();
        return service.codeSendList(uid);
    }
}