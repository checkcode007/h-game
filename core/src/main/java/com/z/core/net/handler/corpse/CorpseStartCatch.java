 ...
 */
@Service
public class CorpseStartCatch implements IHandler<Game.C_20321> {
    @Autowired
    ...
    public AbstractMessageLite handle(ChannelHandlerContext ctx, MyMessage.MyMsgReq msgReq) throws Exception {
        ...
        Game.C_20321  req =  Game.C_20321.parseFrom(ByteString.copyFrom(list).toByteArray());
        return handleDo(ctx,req);
    }

    @Override
    public AbstractMessageLite handleDo(ChannelHandlerContext ctx, Game.C_20321 req) {
        long uid = ctx.channel().attr(ChannelAttributes.USER_ID).get();
        return service.corPseStart(uid);
    }
}
