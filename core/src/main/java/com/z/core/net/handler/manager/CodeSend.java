 ...
 */
@Component
public class CodeSend implements IHandler<User.C_10409> {
    ...
    public AbstractMessageLite handle(ChannelHandlerContext ctx, MyMessage.MyMsgReq msgReq) throws Exception {
        ...
        User.C_10409 req = User.C_10409.parseFrom(ByteString.copyFrom(list).toByteArray());
        return handleDo(ctx, req);
    }

    @Override
    public AbstractMessageLite handleDo(ChannelHandlerContext ctx, User.C_10409 req) {
        long uid = ctx.channel().attr(ChannelAttributes.USER_ID).get();
        return service.codeSend(uid, req.getGiveUid(),req.getCount());
    }
}